...
PROGRAM_MAP_VERSION=0.26
PROGRAM_MAP_YAML_VERSION=0.26
PROGRAM_MAP_ACCEPTANCE_SUMMARY_VERSION=0.26
...
Эта current projection сохраняет C1–C8 PASS и hardened target-state SSOT: AIFE Server владеет generic collection runtime, Data Bridge сохраняет provider/domain semantics, standalone legacy D8 остаётся running unchanged pre-cutover и подлежит separately-authorized controlled retirement только после proven F8 cutover. C9 не запускался и fresh exact-head reauthorization ожидает owner audit; C10/F5M/F8/production cutover/legacy retirement execution не выполнялись.
