...
Эта control projection фиксирует physical C8 terminal PASS и fresh live D8 provenance PASS. C9 не запускался. Fresh exact-head C9 authorization predecessor run на `eb69e6b35ccbe109b16562895a1dd4e547bb88c2` superseded этим control-semantic repair; этот successor одновременно materialize-ит новую fresh exact-head authorization, bound к audited entry predecessor, C8 source/release/executor и fresh D8 source/image/config identities. Self-publication этого successor не является invalidating drift; C9 execution остаётся отдельным run после owner audit этого successor. F5M, AEB generation и production cutover здесь не выполняются.
