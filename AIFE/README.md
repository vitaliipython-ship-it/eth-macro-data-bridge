...
Эта transitional projection сохраняет C1–C7 PASS, фиксирует R02 как consumed/blocker-only authority и требует exact-SHA qualification + control successor для repaired privileged-executor bytes. Старый R02 не авторизует новые source bytes; C8 execution, VPS mutation, F5M, real AIFE mutation, AEB generation и production cutover здесь не авторизуются.
