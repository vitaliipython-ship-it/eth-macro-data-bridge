...
NEXT_AGENT_ACTION_COUNT=MINIMIZED
...
Эта control projection фиксирует hardened target operating state: generic collection runtime/durability authority принадлежит AIFE Server/AIFE durable lifecycle, provider/domain semantics остаются authority ETH Macro Data Bridge, а standalone D8 — только pre-cutover compatibility predecessor с обязательным separately-authorized controlled retirement после proven F8 cutover. C9 не запускался. Authorization predecessor `490c96623d8ada995dad9ddfe2777d4a5b78be64` superseded target-state SSOT hardening; этот successor одновременно materialize-ит новую fresh exact-head C9 authorization, bound к audited entry predecessor, unchanged C8 source/release/executor и unchanged live D8 source/image/config identities. Self-publication этого successor не является invalidating drift; C9 execution остаётся отдельным run после owner audit. F5M/F8/retirement execution/production cutover здесь не выполняются.
