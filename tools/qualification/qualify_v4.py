...
def main():
    before=snap(); old_counts=counts(); subprocess.run([sys.executable,"src/collector.py"],check=True); after=snap(); new_counts=counts()
    ...
    subprocess.run([sys.executable,"tools/validation/validate.py"],check=True); subprocess.run([sys.executable,"tools/validation/validate_v4.py"],check=True)
