...
def qualify_segment_b(output_root: Path, restored_left: Path, repository_root: Path = Path(".")) -> dict:
    ...
    warm_first_ms = _warm_first_timestamp(repository_root); cutoff_ms = int(json.loads((repository_root / "history" / "release-manifest.json").read_text())["backfill_as_of_ms"]); warm_start_ms = _utc_day_start_ms(warm_first_ms); warm_end_ms = min(cutoff_ms, warm_start_ms + WARM_OVERLAP_MS)
