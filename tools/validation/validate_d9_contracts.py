...
def main() -> None:
    ...
    raw_ptr = contract["semantic_contracts"].get("raw_chain_transfer_fact")
    require(isinstance(raw_ptr, dict), "raw chain-transfer semantic contract pointer missing")
    require(raw_ptr == {"analytical_state":False,"capability_id":"blockchain.raw-transfer-facts","contract_id":"ETH-RAW-CHAIN-TRANSFER-FACT-SEMANTIC-CONTRACT-V1","layer":"L0_FACTUAL","owner_domain":"MARKET_DATA_FOUNDATION","path":"contracts/raw-chain-transfer-fact-semantic-contract-v1.json","provider_selection":"DEFERRED","runtime_active":False,"selective_v2_required":True,"status":"CONTRACT_BOUND_RUNTIME_ROUTE_NOT_IMPLEMENTED"}, "raw chain-transfer bridge pointer mismatch")
    raw = read(raw_ptr["path"])
    require(raw["contract_id"] == "ETH-RAW-CHAIN-TRANSFER-FACT-SEMANTIC-CONTRACT-V1", "raw transfer contract id mismatch")
    require(raw["schema_version"] == "eth-raw-chain-transfer-fact-semantic-contract/1.0.0", "raw transfer contract version mismatch")
    require(raw["authority"]["owner_domain"] == "MARKET_DATA_FOUNDATION" and raw["authority"]["layer"] == "L0_FACTUAL", "raw transfer owner/layer mismatch")
    require(raw["authority"]["raw_transfer_fact_does_not_imply_entity_attribution"] is True, "raw transfer fact implies entity attribution")
    require(raw["capability"]["capability_id"] == "blockchain.raw-transfer-facts" and raw["capability"]["provider_selection"] == "DEFERRED", "raw transfer capability identity/provider boundary mismatch")
    require(raw["capability"]["runtime_active"] is False and raw["capability"]["route_status"] == "NOT_IMPLEMENTED", "raw transfer runtime activated")
    plan = raw["resolution_plan"]
    require(plan["plan_schema"] == "market-data-resolution-plan/2.0.0" and plan["series_kind"] == "STRUCTURED_TIME_SERIES" and plan["coverage_semantics"] == "EVENT_DRIVEN", "raw transfer ResolutionPlan semantics mismatch")
    require(plan["finality_policy"] == "PROVISIONAL_ALLOWED_EXPLICITLY" and plan["revision_policy"] == "CHAIN_CANONICALITY_REVISION", "raw transfer finality/revision semantics mismatch")
    require(plan["d9_global_activation"] is False and plan["v2_global_activation"] is False and plan["canonicality_is_finality"] is False, "raw transfer activation/finality boundary mismatch")
    require(plan["old_transfer_observation_deleted"] is False and plan["old_transfer_observation_overwritten"] is False and plan["superseded_transfer_remains_addressable"] is True, "raw transfer append-only reorg semantics mismatch")
    require(raw["coverage"]["absence_without_coverage_proof_is_zero"] is False, "raw transfer absence silently became zero")
    boundaries = raw["boundaries"]
    require(boundaries["provider_selected"] is False and boundaries["rpc_selected"] is False and boundaries["indexer_selected"] is False and boundaries["storage_selected"] is False, "raw transfer selected provider/RPC/indexer/storage")
    require(boundaries["network_calls_added"] == 0 and boundaries["history_capability_index_runtime_row_created"] is False, "raw transfer runtime/catalog activation leaked")
    history_index = read("history/capability-index.json")
    require("blockchain.raw-transfer-facts" not in json.dumps(history_index, sort_keys=True), "raw transfer fake runtime capability row exists")
    ...
    print("RAW_TRANSFER_FACT_SEMANTIC_CONTRACT=PASS")
    print("RAW_TRANSFER_FACT_CAPABILITY_IDENTITY_BOUND=PASS")
    print("RAW_TRANSFER_FACT_RUNTIME_ROUTE=NOT_IMPLEMENTED")
