...
from r13_continuity import verify_segment,verify_seam,verify_segment_023,GAP
...
coverage_end_ms=int(p.parse_utc(evidences[-1]["requested_end_utc"]).timestamp()*1000); warm=b.verify_warm_overlap_records(build0,Path("."),coverage_end_ms=coverage_end_ms)
...
repair_payload=json.loads(Path("history/kraken/ETHUSD/1d/2026.json").read_text())
repaired_core=next(r for r in repair_payload["records"] if int(r[0])==1782172800000)
repaired_native=next(r for r in repair_payload["provider_native_records"] if int(r[0])==1782172800000)
cold_repaired=next(r for r in build0["1d"] if int(r[0])==1782172800000)
if any(not b._numeric_equal(repaired_core[i], cold_repaired[i]) for i in range(1,6)) or int(repaired_native[7])!=int(cold_repaired[6]): raise RuntimeError("R13 exact warm repair mismatch")
aug15=next(r for r in build0["1d"] if int(r[0])==1786752000000)
aug_eligible,aug_reason=b._warm_overlap_eligibility(aug15,"1d",coverage_end_ms)
if aug_eligible or aug_reason!="PARTIAL_SOURCE_COVERAGE": raise RuntimeError("R13 partial terminal day eligibility mismatch")
r12_class=json.loads((Path(os.environ["RUNNER_TEMP"])/"r12"/"classification.json").read_text())["root_cause_classification"]
if r12_class!="R12-C4_WARM_CANONICAL_DATA_DEFECT": raise RuntimeError("R12 root-cause authority drift")
repair_evidence={"r12_root_cause":r12_class,"warm_repaired_timestamp":1782172800000,"warm_repaired_status":"REPAIRED_AND_MATCHED","partial_terminal_1d_timestamp":1786752000000,"partial_terminal_1d_status":"NOT_COMPARABLE_PARTIAL_COLD_BUCKET","partial_terminal_reason":aug_reason,"coverage_end_ms":coverage_end_ms,"warm_overlap":warm,"provider_network_calls":0}
...
"acquired_at_utc":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"execution_lineage":{"R07":{"run_id":R07,"logical_segments":"000..021"},"R08":{"run_id":R08,"logical_segments":"022"},"R09":{"run_id":R09,"logical_segments":"023..044","orchestration_sha":R09SHA},"R13":{"run_id":int(os.environ["GITHUB_RUN_ID"]),"orchestration_sha":os.environ["GITHUB_SHA"],"provider_network_calls":0}}}
(candidate/"source.json").write_bytes(b.compact(source))
(candidate/"r13-repair-evidence.json").write_bytes(b.compact(repair_evidence))
...
warm_assets=b.verify_warm_overlap(assets_a,coverage_end_ms=coverage_end_ms)
if warm_assets["conflicts"]!=0 or warm_assets["overlaps"]!=warm["overlaps"]: raise RuntimeError("assembled-output/Build-A WARM mismatch")
...
inv={"schema_version":"kraken-posttrade-full-candidate-inventory/3.0.0","source_authority_sha":SRC,"source_authority_tree":TREE,"r13_orchestration_sha":os.environ["GITHUB_SHA"],"r13_run_id":int(os.environ["GITHUB_RUN_ID"]),"provider_network_calls":0,
...
inv.update({"r12_root_cause":"R12-C4_WARM_CANONICAL_DATA_DEFECT","warm_repair":{"timestamp":1782172800000,"status":"REPAIRED_AND_MATCHED"},"partial_terminal_day":{"timestamp":1786752000000,"status":"NOT_COMPARABLE_PARTIAL_COLD_BUCKET","reason":"PARTIAL_SOURCE_COVERAGE"},"warm_repair_scope":"EXACT_TIMESTAMP_1782172800000_ONLY","warm_canonical_data_mutated":True,"warm_verifier_mutated":True})
...
qualification.update({"r12_root_cause":"R12-C4_WARM_CANONICAL_DATA_DEFECT","warm_partial_buckets_skipped_5m":warm["partial_buckets_skipped"]["5m"],"warm_partial_buckets_skipped_1d":warm["partial_buckets_skipped"]["1d"],"warm_repair_scope":"EXACT_TIMESTAMP_1782172800000_ONLY","warm_canonical_data_mutated":True,"warm_verifier_mutated":True})
...
print("R13_EXACT_2026_06_23_WARM_CORE_ROW=PASS")
print("R13_EXACT_2026_06_23_NATIVE_ROW=PASS")
print("PARTIAL_1D_BUCKET_BOUNDARY_REGRESSION=PASS")
print("COMPLETE_5M_BUCKET_BOUNDARY_REGRESSION=PASS")
print("REAL_COMPLETE_CONFLICT_FAIL_CLOSED=PASS")
print(f"WARM_OVERLAPS_5M={warm['overlaps']['5m']}")
print(f"WARM_OVERLAPS_1D={warm['overlaps']['1d']}")
print(f"WARM_PARTIAL_BUCKETS_SKIPPED_5M={warm['partial_buckets_skipped']['5m']}")
print(f"WARM_PARTIAL_BUCKETS_SKIPPED_1D={warm['partial_buckets_skipped']['1d']}")
print("WARM_OVERLAP_CONFLICTS=0")
print("2026-06-23_1D_STATUS=REPAIRED_AND_MATCHED")
print("2026-08-15_1D_STATUS=NOT_COMPARABLE_PARTIAL_COLD_BUCKET")
print("BUILD_A_5M=PASS"); print("BUILD_B_5M=PASS"); print("BUILD_A_B_5M_EQUAL=PASS")
print("BUILD_A_1D=PASS"); print("BUILD_B_1D=PASS"); print("BUILD_A_B_1D_EQUAL=PASS")
