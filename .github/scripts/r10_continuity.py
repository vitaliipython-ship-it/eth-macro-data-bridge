...
def verify_segment(e):
    ...
    # Do NOT require final_cursor >= requested_end_utc.
    # Exact pinned source also marks COMPLETE when the provider returns a short
    # final page, which proves exhaustion/no-trade tail for the query window.
    return True
...
def verify_seam(left,right):
    ...
    # Each fixed UTC segment is an independent PostTrade pagination chain.
    # previous.final_cursor == next.initial_cursor is not a contract.
    return True
