...
def source() -> dict:
    return {
        ...
        "file_id": backfill.FILE_ID,
        "landing_url": backfill.LANDING_URL,
...
class KrakenSpotOhlcvtBackfillTests(unittest.TestCase):
    def test_confirmation_form_and_quota_classification(self):
        html = b'''<form action="https://drive.usercontent.google.com/download" method="get">
        <input name="id" value="abc"><input name="export" value="download">
        <input name="confirm" value="t"><input name="uuid" value="u-1"></form>'''
        url = backfill._confirmed_download_url(html)
        self.assertIn("id=abc", url)
        self.assertIn("uuid=u-1", url)
        with self.assertRaises(backfill.UpstreamQuotaBlocked):
            backfill._classify_html_response(b"<title>Google Drive - Quota exceeded</title>")
    ...
    def test_build_is_deterministic_and_accepts_legacy_pair_alias(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            payload = json.loads(next(item for item in left if item["interval_or_metric"] == "5m")["local_path"] and Path(next(item for item in left if item["interval_or_metric"] == "5m")["local_path"]).read_text())
