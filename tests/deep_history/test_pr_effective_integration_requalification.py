...
class LateBoundRequalificationTests(unittest.TestCase):
    ...
    def test_19_workflow_evidence_path_outside_repository_root(self):
        s=WORKFLOW.read_text(); self.assertNotIn('EVIDENCE_PATH: ${{ runner.temp }}/pr-effective-integration-qualification.json',s); self.assertNotIn('EVIDENCE_PATH: pr-effective-integration-qualification.json',s); self.assertIn('EVIDENCE_PATH="$RUNNER_TEMP/pr-effective-integration-qualification.json"',s); self.assertIn('>> "$GITHUB_ENV"',s); self.assertIn('case "$EVIDENCE_PATH" in',s); self.assertIn('"$GITHUB_WORKSPACE"|"$GITHUB_WORKSPACE"/*)',s)
