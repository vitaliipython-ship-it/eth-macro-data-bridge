...
from kraken_trade_flow import (
    classify_root_cause,
    collect_trade_flow_evidence,
    gate_native_trade_metrics,
    normalize_trade,
)
...
def getter(history, server_time=None):
    def get(url):
        return {
            "result": "success",
            "serverTime": server_time or iso(END + 1_000),
...
class KrakenTradeFlowTests(unittest.TestCase):
    def test_1_valid_trades_count_and_volume(self):
        evidence = collect_trade_flow_evidence(
            getter(
                [
                    trade(1_000, "2", "buy", trade_id=1),
                    trade(2_000, "3", "sell", trade_id=2),
                    trade(-1_000, "1", "buy", trade_id=3),
                ]
            ),
            NOW,
            "PI_ETHUSD",
    ...
    def test_2_balanced_aggressor_flow_zero_is_not_no_data(self):
        evidence = collect_trade_flow_evidence(
            getter(
                [
                    trade(1_000, "4", "buy", trade_id=1),
                    trade(2_000, "4", "sell", trade_id=2),
                    trade(-1_000, "1", "buy", trade_id=3),
                ]
            ),
            NOW,
    ...
    def test_3_no_trades_complete_observation_is_valid_zero(self):
        evidence = collect_trade_flow_evidence(
            getter([trade(-1_000, "1", "buy", trade_id=99)]), NOW, "PI_ETHUSD"
        )
        ...
        self.assertEqual(classify_root_cause(evidence), "VALID_ZERO_NO_TRADES_IN_BUCKET")
    ...
    def test_4_feed_unavailable_fails_closed(self):
        ...
        metrics = {
            name: {"latest": [END, 0], "freshness_status": "LIVE_USABLE"}
            for name in ("trade-count", "trade-volume", "aggressor-differential", "cvd")
        }
        gate_native_trade_metrics(metrics, evidence)
        self.assertTrue(
            all(
                item["latest"] is None and item["availability_status"] == "UNAVAILABLE"
                for item in metrics.values()
            )
        )
        self.assertTrue(all(item["native_latest"] == [END, 0] for item in metrics.values()))

    def test_5_wrong_product_is_observable_not_zero_market(self):
        evidence = collect_trade_flow_evidence(
            getter(
                [
                    trade(1_000, "2", "buy", "PF_ETHUSD", 1),
                    trade(-1_000, "1", "buy", "PI_ETHUSD", 2),
                ]
            ),
            NOW,
            "PI_ETHUSD",
    ...
    def test_6_bucket_boundaries_are_start_inclusive_end_exclusive(self):
        rows = [
            trade(-1, "1", "buy", trade_id=1),
            trade(0, "1", "buy", trade_id=2),
            trade(150_000, "1", "sell", trade_id=3),
            trade(300_000, "1", "buy", trade_id=4),
        ]
    ...
    def test_8_unknown_side_is_never_silently_buy_or_sell(self):
        evidence = collect_trade_flow_evidence(
            getter(
                [
                    trade(1_000, "2", "mystery", trade_id=1),
                    trade(-1_000, "1", "buy", trade_id=2),
                ]
            ),
            NOW,
            "PI_ETHUSD",
    ...
    def test_10_cvd_is_not_recomputed_or_reset_by_gate(self):
        evidence = {
            ...
            "bucket_start": START,
            "bucket_end": END,
            "raw_trade_message_count": 2,
            "bucketed_trade_count": 2,
        }
    ...
    def test_11_live_pattern_old_trades_complete_bucket_is_valid_zero(self):
        evidence = collect_trade_flow_evidence(
            getter(
                [
                    trade(-9 * 3_600_000, "2", "buy", trade_id=1),
                    trade(-8 * 3_600_000, "1", "sell", trade_id=2),
                ]
            ),
            NOW,
            "PI_ETHUSD",
        ...
        self.assertTrue(evidence["coverage_complete"])
        self.assertEqual(evidence["bucketed_trade_count"], 0)
        self.assertEqual(evidence["drop_reason_counts"]["BEFORE_BUCKET_START"], 2)
        self.assertEqual(classify_root_cause(evidence), "VALID_ZERO_NO_TRADES_IN_BUCKET")

    def test_decision_tree_a_when_no_observation(self):
        self.assertEqual(
            classify_root_cause(
                {"feed_observed": False, "raw_trade_message_count": 0, "coverage_complete": False}
            ),
            "A_ACQUISITION_OR_COVERAGE",
        )
