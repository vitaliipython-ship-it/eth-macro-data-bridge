...
import collector
from d8_runtime import D8Runtime
...
class StaleSlotAcquisitionRegression(unittest.TestCase):
    ...
    def _fake_spot(self, _symbol, _interval, _limit, _physical_now_ms, *, anchor_ms=None):
    ...
    def _collect(self, capability: str, actual_ms: int) -> dict:
        ...
        with patch(target, side_effect=self._fake_spot):
    ...
    def test_temporal_unavailability_is_not_connectivity(self):
        self.assertEqual(
            D8Runtime._classify_exception(ValueError("no eligible finalized observation for requested slot")),
            "VALIDATION_FAILED",
        )
