...
from intelligence import VERSION, depth_metrics, flatten_kraken
...
def validate():
    ...
    d=json.loads(Path("derivatives/manifest.json").read_text()); assert d["providers"]["binance-usdm"]["status"]=="PASS"
    assert d["providers"]["kraken-futures"]["status"]=="PASS"
    seen=set()
    ...
    lm=json.loads(Path("liquidity/manifest.json").read_text()); lp=lm["providers"]["baseline"]; assert lp["status"]=="PASS"
...
if __name__=="__main__": validate(); fixtures()
